...
Полноценное веб-приложение для управления фитнес-клубом.
Проект охватывает все основные концепции Spring Boot: безопасность, JPA, миграции БД, REST API, фоновые задачи, статический фронтенд, контейнеризация и централизованное логирование.
...
| Сбор логов | Loki + loki-logback-appender | 3.0.0 / 1.5.2 |
| Визуализация логов | Grafana | 11.0.0 |
...
│       └── provisioning/
│           ├── datasources/
│           │   └── loki.yml                   # Автоподключение Loki как datasource
│           └── dashboards/
...
2. startDateTime <= now() + 30 дней           → BusinessException
...
**Пессимистичная блокировка** при проверке конфликтов во время создания брони:
```java
@Lock(LockModeType.PESSIMISTIC_WRITE)
List<TrainingBooking> findConflictingBookings(...);
```
Это гарантирует, что два одновременных запроса на один слот не пройдут оба — один будет ждать снятия блокировки.

> **Важно**: для чтения занятых слотов (`getFreeSlots`) используется отдельный метод
> `findOccupiedBookings` — без блокировки. Это необходимо, потому что `getFreeSlots` работает
> в `@Transactional(readOnly = true)` транзакции, а PostgreSQL запрещает `SELECT FOR UPDATE`
> внутри read-only транзакции.
...
### Форма программы тренировок (trainer.html)

Редактор упражнений отображает подписи колонок над полями ввода:

| Поле | Пример значения |
|------|----------------|
| Название упражнения | Жим лёжа |
| Подходы | 4 |
| Повторения | 8–12 |
| Вес (кг или описание) | 60 кг / без веса |
| День | 1 |

### Программа тренировок на стороне клиента (client.html)

Раздел «Программа» показывает:
- **Сводку**: количество дней, упражнений и суммарное число подходов
- **Карточки по дням** — в заголовке каждой: день и кол-во упражнений/подходов
- **Нумерацию упражнений** внутри дня
- **Бейджи**: подходы — синий, повторения — голубой с `×`, вес — серый
- Сортировку дней по порядку, упражнений — по `orderIndex`

### Бронирование слотов (client.html)

Инпут выбора даты ограничен диапазоном **сегодня + 30 дней**.
Дата по умолчанию устанавливается в **локальном времени браузера** (не UTC), чтобы избежать смещения на сутки в часовых поясах, отличных от UTC.
...
> При локальном запуске (без Docker) Loki-аппендер не активируется — логи пишутся только в консоль.
...
| URL | Описание |
|-----|---------|
| http://localhost:8091 | Приложение |
| http://localhost:8091/swagger-ui.html | Swagger UI |
| http://localhost:3000 | Grafana (логи) — admin / admin |

### Управление контейнерами

```bash
docker compose down          # остановить, данные сохранятся в volumes
docker compose down -v       # остановить + удалить все volumes (сброс БД и логов)
docker compose up --build -d # пересобрать образ после изменений кода
```
...
### Spring-профиль docker

В `docker-compose.yml` установлена переменная `SPRING_PROFILES_ACTIVE: docker`.
Она активирует Loki-аппендер в `logback-spring.xml`, который отправляет все логи в Loki по HTTP.
При локальном запуске без этой переменной Loki-аппендер не подключается.
...
```
Spring Boot (logback)
       │  loki4j appender (HTTP push)
       ▼
    Loki :3100  ← хранит логи в /loki/chunks (volume)
       │
       ▼
  Grafana :3000 ← читает логи через Loki datasource
```

Grafana и Loki datasource настраиваются **автоматически** через provisioning-файлы в `docker/grafana/`.
Никаких ручных действий в UI не требуется.

### Открыть дашборд

1. Перейди на http://localhost:3000
2. Войди: логин `admin`, пароль `admin`
3. Слева → **Dashboards** → папка **Pet Gym** → **Pet Gym — Логи**

### Что показывает дашборд

| Элемент | Описание |
|---------|---------|
| Счётчики ошибок/предупреждений | Сколько ERROR и WARN за выбранный период |
| График по уровням | Частота логов INFO / DEBUG / WARN / ERROR во времени |
| Лог-панель | Все логи в реальном времени с деталями (разворачиваются по клику) |
| Поле «Поиск» | Фильтрует логи по ключевому слову (email, класс, текст ошибки) |

Дашборд обновляется каждые **10 секунд** автоматически.

### Как искать нужное

Поле поиска поддерживает любую подстроку из текста лога:

```
ERROR               — только ошибки
BookingService      — логи конкретного класса
client1@fit.com     — действия конкретного пользователя
500                 — внутренние ошибки сервера
Booking created     — успешные бронирования
```

Временной диапазон меняется в правом верхнем углу (Last 1 hour / Last 24 hours и т.д.).
...
### `500 Internal Server Error` при запросе свободных слотов

**Причина**: Метод `getFreeSlots` был помечен как `@Transactional(readOnly = true)`, но вызывал
`findConflictingBookings`, у которого стоит `@Lock(PESSIMISTIC_WRITE)`.
PostgreSQL запрещает `SELECT FOR UPDATE` в read-only транзакции.

**Решение (уже применено)**: Для чтения занятых слотов используется отдельный метод
`findOccupiedBookings` без блокировки. Пессимистичная блокировка остаётся только в методе создания брони.

---

### Дата «сегодня» смещается на вчера

**Причина**: `new Date().toISOString()` возвращает UTC-время. Если пользователь в часовом поясе
UTC+3 и время до 03:00, ISO-строка даёт вчерашнюю дату.

**Решение (уже применено)**: Дата инициализируется через локальные методы браузера:
```javascript
const now = new Date();
...
3. Обращение к чужой роли (`CLIENT` → `/api/admin/...` вернёт 403, не 401)

---

### `BookingConflictException` — тренер уже занят

```bash
# Попробуй забронировать одно и то же время дважды:
POST /api/client/bookings
{ "trainerId": 3, "startDateTime": "2026-05-10T10:00:00" }
...
Возможные причины:
1. Приложение ещё не успело запуститься — подожди 1–2 минуты после `docker compose up`
